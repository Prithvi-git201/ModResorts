...
public class WeatherServlet extends HttpServlet {
   ...
   * for containerized deployment. Use SERVER_NAME and SERVER_FULL_NAME environment variables.
   */
  private String configureEnvDiscovery() {
    String serverEnv = "";
    
    // Replace WebSphere-specific APIs with environment variables for container deployment
  ...
  /**
   * Removed RMI-based InitialContext configuration (IIOP/CORBA).
   * For containerized deployment, use REST/gRPC with service discovery (AWS Cloud Map, Consul, etc.)
   * or direct service-to-service communication via environment variables.
   * 
   * This method is deprecated and should not be used in container environments.
   */
  @Deprecated
  private void setInitialContextProps() {
    // RMI/IIOP not supported in containerized environments
    // Use REST APIs with service discovery instead
    logger.warning("RMI-based naming context is not supported in containerized environments. " +
                   "Use REST/gRPC with service discovery (AWS Cloud Map, Consul) instead.");
  }
}
